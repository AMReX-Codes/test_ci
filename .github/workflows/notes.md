...
There are two cache cleanup workflows, cleanup-cache.yml and
clean-up-cache-postpr.yml.
...
run on the default branch (main in this example) of the fork. Note that the
CI here runs on the fork, not the main repo. However, if a PR of this branch
has been submitted to the main repo, it will also trigger the PR workflows
on the main repo.
...
The merge closes the PR and therefore triggers the cleanup-cachexxx
The merge will also update the default branch on the main repo. This is
considered push event even though it's not pushed by a normal user. So this
will trigger all the push triggered workflows (e.g., cuda.yml), which will
in turn trigger the clean-up-cache.
